...
/** id массива designChecklist — text, как у других array-полей Payload. */
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.run(sql`DROP TABLE IF EXISTS \`components_design_checklist\`;`);

  await db.run(sql`CREATE TABLE \`components_design_checklist\` (
