/**
 * Сниппеты и таблицы пропсов для первичного наполнения CMS (`npm run seed:portal`).
 * Не импортировать из портала в runtime — на сайте источник правды Payload CMS.
 */

export type BootstrapPropRow = {
  name: string;
  type: string;
  default?: string;
  description: string;
...
export type ComponentCodeSnippets = {
  label: string;
  props: BootstrapPropRow[];
  importSnippet: string;
  basicSnippet: string;
  variantSnippets: { label: string; code: string }[];
};

export const componentCodeSnippetsBySlug: Record<string, ComponentCodeSnippets> = {
  button: {
    label: "Button",
