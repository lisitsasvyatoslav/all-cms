...
import {
  applyPortalAppearanceToDocument,
  readPortalAppearance,
...
const { appearance: _appearance, ...radixThemePropsWithoutAppearance } = portalRadixThemeProps;

export function PortalThemeProvider({ children }: { children: ReactNode }) {
  const [appearance, setAppearanceState] = useState<PortalAppearance>("light");
  const [ready, setReady] = useState(false);

  useEffect(() => {
    const initial = readPortalAppearance();
    setAppearanceState(initial);
    applyPortalAppearanceToDocument(initial);
    setReady(true);
  }, []);

  const setAppearance = useCallback((next: PortalAppearance) => {
    setAppearanceState(next);
    try {
      localStorage.setItem(PORTAL_APPEARANCE_STORAGE_KEY, next);
    } catch {
      /* ignore */
    }
    applyPortalAppearanceToDocument(next);
  }, []);
